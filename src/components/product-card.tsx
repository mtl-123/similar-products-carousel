    ...
    <article className="group min-w-0">
      <div className="relative aspect-[4/5] overflow-hidden rounded-[5px] bg-[#ecece8]">
        <Link href={`/product/${product.slug}`}>
          <Image src={product.image} alt={text.name} fill unoptimized={product.image.startsWith("/api/uploads/")} sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="object-cover transition duration-500 group-hover:scale-[1.025]" />
        </Link>
